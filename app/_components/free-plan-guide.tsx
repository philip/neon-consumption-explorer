import { getProjects, getProjectBranches } from "@/lib/api"
...
export async function FreePlanGuide({ orgId }: { orgId: string }) {
  ...
  const branchResults = await Promise.allSettled(
    projects.map(async (p) => {
      ...
      return { projectId: p.id, branches }
    }),
  )
  ...
  for (const r of branchResults) {
    if (r.status === "fulfilled") {
      branchMap.set(r.value.projectId, r.value.branches)
    }
  }
  ...
  for (const p of projects) {
    maxComputeSeconds = Math.max(maxComputeSeconds, p.compute_time_seconds ?? 0)
    totalTransferBytes += p.data_transfer_bytes ?? 0
  }

  const allowances = getPlan("free").allowances
  const computeHoursLimit = allowances.computeCUHoursPerProject ?? 100
  const storagePerProjectBytes = (allowances.storageGBPerProject ?? 0.5) * 1e9
  const transferLimitBytes = allowances.publicTransferGB * 1e9
        ...
        <Card className="mt-2">
          <CardContent className="space-y-3 pt-4">
            <div className="space-y-2 text-sm text-muted-foreground">
              <p>
                <strong className="text-foreground">List projects</strong>{" "}
                <code>active_time_seconds</code>, <code>compute_time_seconds</code>,{" "}
                <code>data_transfer_bytes</code>
              </p>
              <CurlBlock
                cmd={`curl "https://console.neon.tech/api/v2/projects?org_id=\${ORG_ID}&limit=100" \\
